...
zara_product=[z.get("title")for z in clothes if z.get("brand") == "Zara"]
print(zara_product)
# display all the product titles available in size s
size = [s.get("title") for s in clothes if "S" in s.get("sizes")]
print(size)
mostnoprct = [p.get("brand") for p in clothes]
brand_count={b:mostnoprct.count(b) for b in mostnoprct}
print(brand_count)
