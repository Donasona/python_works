...
print(len(countries))
all_countries_names=[c.get("name") for c in countries]
...
# create a fn get_country_data_by_name(country_name)
# return data of countries with name =country_name

def get_country_by_name(country_name):
    return[c for c in countries if c.get("name").lower()==country_name]
# print(get_country_by_name("india"))

# create a function get_country_capital(country_name)
...
def get_currency_symbol(country_name):
    country_data=[c for c in countries if c.get("name").lower()==country_name][0]
    if "currencies" in country_data:
        all_correncies=country_data.get("currencies")
        for cur in all_correncies:
            print(cur.get("symbol"))
get_currency_symbol("india")

def get_contry_with_english_lang():
    for c in countries:
        if "languages" in c:
            for lang in c.get("languages"):
                if lang.get("name").lower()=="english":
                    print(c.get("name"))
get_contry_with_english_lang()

longest_cntry=max(countries,key=lambda dictionary:dictionary.get("area",0))
print(longest_cntry.get("name"))

most_populated_country=max(countries)